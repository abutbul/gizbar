            ...
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex items-center justify-between h-16">
                    <div className="flex items-center space-x-2">
                        ...
                        <h1 className="text-xl font-bold text-slate-800 dark:text-slate-100">{t('appName')}</h1>
                        <div className="relative" ref={langMenuRef}>
                            <button onClick={() => setLangMenuOpen(prev => !prev)} className="flex items-center space-x-1 p-2 rounded-md hover:bg-slate-100 dark:hover:bg-slate-800">
                                <span>{languages[language].flag}</span>
                                <ChevronDownIcon className="w-4 h-4 text-slate-500" />
                            </button>
                            {isLangMenuOpen && (
                                <div className="absolute top-full mt-2 w-40 rounded-md shadow-lg bg-white dark:bg-slate-800 ring-1 ring-black ring-opacity-5 py-1">
                                    {Object.keys(languages).map((langKey) => (
                                        <button 
                                            key={langKey} 
                                            onClick={() => { setLanguage(langKey as Language); setLangMenuOpen(false); }}
                                            className="w-full text-start flex items-center space-x-3 px-4 py-2 text-sm text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700"
                                        >
